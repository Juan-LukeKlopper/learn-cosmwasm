...
Bytecode can be downloaded through `CODE_ID` and verified by comparing it with its own bytecode. Do `diff`.
...
You can call the method or request a state change for this instance (execute) or get the data (query) after the instantiation.

```bash
# execute fails if wrong person
REGISTER='{"register":{"name":"fred"}}'
wasmd tx wasm execute $CONTRACT "$REGISTER"     --amount 100upebble     --from wallet $TXFLAG -y
...
wasmd tx wasm execute $CONTRACT "$TRANSFER"     --amount 999upebble     --from wallet $TXFLAG -y
```